...
if __name__ == '__main__':
    ...
    # ***************************************************
    B = Adafruit_MCP230XX(address = 0x24, num_gpios = 16) # MCP23017
    A = Adafruit_MCP230XX(address = 0x20, num_gpios = 16)
    for i in range(16): 
      A.config(i, A.OUTPUT)
      time.sleep(.1)
      A.output(i, 0)
...
class McpController():
  ...
  def __init__(self, pin) -> None:
      ...
      mcp_name = pin[0]
      mcp = self.gpio_extenders[mcp_name]
      self.mcp = mcp
  ...
  def on(self):
      ...
      self.log("Turned on mcp {} pin {} to {}".format(self.mcp_name, self.pin, 1))
      self.mcp.output(self.pin, 1)
  
  def off(self):
      self.state = 0
      self.log("Turned on mcp {} pin {} to {}".format(self.mcp_name, self.pin, 0))
...
def load(control, log_f, config):
  McpController.log = log_f
  for name, address in config.items():
      try: 
          gpio_extenders[name] = Adafruit_MCP230XX(address = address, num_gpios = 16)
          for i in range(16): gpio_extenders[name].config(i, gpio_extenders[name].OUTPUT)
          for i in range(16): gpio_extenders[name].output(i, 0)
          log_f(f"Loaded gpio extender {name}")
      except Exception as err:
          log_f(f"Unable to load gpio extender {name} - {err}")

  return {"McpController": McpController, "gpio_extenders": gpio_extenders}

def unload(control, log_f, config, gpio_extenders, McpController):
    for name, mcp in gpio_extenders.items():
        for i in range(16): mcp.output(i, 0)
    
        log_f(f"Cleaned gpio extender {name}")
